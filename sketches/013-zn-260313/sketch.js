// ======================================================
// GEN - Stronger White Core Version
// p5.js sketch.js
// ======================================================
...
function drawCenterStar(cx, cy, t, intensity) {
  const p = pulseValue(t, CONFIG.pulseCycle, 1.55);

  const coreSize = lerp(CONFIG.centerCoreMin, CONFIG.centerCoreMax, p);
  const coreAlpha = lerp(215, 255, p) * lerp(0.95, 1.0, intensity);

  const halo1Size = lerp(CONFIG.halo1Min, CONFIG.halo1Max, p);
  const halo1Alpha = lerp(12, 34, p);

  const halo2Size = lerp(CONFIG.halo2Min, CONFIG.halo2Max, p);
  const halo2Alpha = lerp(3, 11, p);

  noStroke();

  // 外側ハローを少し青白く
  fill(235, 242, 255, halo2Alpha);
  ...
  // 十字スパークをさらに明確に
  stroke(255, 255, 255, lerp(70, 145, p));
  strokeWeight(0.95);
  line(cx - 11, cy, cx + 11, cy);
  line(cx, cy - 11, cx, cy + 11);

  // 斜めスパーク
  stroke(245, 248, 255, lerp(16, 42, p));
  strokeWeight(0.5);
  line(cx - 4.8, cy - 4.8, cx + 4.8, cy + 4.8);
  line(cx - 4.8, cy + 4.8, cx + 4.8, cy - 4.8);
}
