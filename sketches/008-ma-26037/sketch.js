...
// Radius breathing version:
// Pulse phase      -> geometry stays near center
// Formation phase  -> radius expands outward
// Rotation phase   -> expanded state rotates slowly
// Fade phase       -> radius contracts inward
// Return           -> back to pulse near center
//
// Scene flow:
// 1. Almost black space
// 2. Small star-like center point
// 3. Slow pulse near center
// 4. Blue-violet geometry emerges
// 5. Mandala expands outward
// 6. Slow rotation in expanded state
// 7. Rotation weakens + contracts inward
// 8. Return to pulse
// 9. Loop
...
function draw() {
  ...
  drawEmergingGeometry(t, geoEnv, pulse, radiusFlow);
  drawMandala(t, mandalaEnv, pulse, radiusFlow);
  ...
  if (DEBUG) {
    drawDebug(t, scene, pulseEnv, geoEnv, mandalaEnv, rotEnv, radiusFlow);
  }
}
...
function drawEmergingGeometry(t, geoEnv, pulse, radiusFlow) {
  if (geoEnv <= 0.001) return;

  const ringCount = floor(lerp(0, 5, geoEnv));

  // ✅ radius breathing reflected here
...
function drawMandala(t, mandalaEnv, pulse, radiusFlow) {
  ...
  for (let L = 0; L < layers; L++) {
    // ✅ inner at pulse / outer at formation / contract later
...
function drawEnsoBarrier(t, scene, radiusFlow) {
  ...
  // ✅ enso radius also breathes with scene
...
function drawDebug(t, scene, pulseEnv, geoEnv, mandalaEnv, rotEnv, radiusFlow) {
  noStroke();
  fill(255);
  textSize(15);
  textAlign(LEFT, TOP);
  text(
    `t=${nf(t, 2, 2)} scene=${scene.name} pulse=${nf(pulseEnv, 1, 2)} geo=${nf(geoEnv, 1, 2)} mandala=${nf(mandalaEnv, 1, 2)} rot=${nf(rotEnv, 1, 2)} radius=${nf(radiusFlow, 1, 2)}`,
    12,
    12
  );
}
